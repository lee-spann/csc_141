...
pygame.display.set_caption("Sideways Shooter with Space Invaders Style")
...
ship_speed = 5.0  # Slow down the ship (float)
bullet_speed = 5.0  # Slow down the bullets (float)
alien_speed = 1.0  # Speed of the aliens
...
class Alien:
    ...
    def update(self):
        ...
        self.rect.x += alien_speed * alien_direction  # Move left or right towards the ship
...
# Set the initial direction of the aliens
alien_direction = -1  # Start with the aliens moving towards the left (towards the ship)

# Create a fleet of aliens on the right side
def create_aliens():
    global aliens
    aliens = []
    for i in range(5):  # Create 5 rows of aliens
        for j in range(10):  # 10 aliens per row
            x = screen.get_width() - (j * 60 + 50)  # Adjust x position for spacing
            y = 150 + (i * 60)  # Adjust y position to create more space between ship and aliens
            aliens.append(Alien(x, y))

create_aliens()  # Initial aliens setup
...
def start_game():
    global game_active, ship_hits, alien_hits, bullets, aliens, score, misses
    game_active = True
    ship_hits = 0
    alien_hits = 0
    score = 0
    ...
    create_aliens()  # Create a new fleet of aliens
    ...
    if game_active:
        ...
        for alien in aliens[:]:
            ...
            if alien.rect.x < 0:  # If the alien reaches the left side, remove it
                aliens.remove(alien)
                misses += 1  # Increment misses when an alien passes the left side

        # If all aliens are destroyed, respawn them and increase difficulty
        if not aliens:
            create_aliens()  # Respawn aliens
            alien_speed += 0.2  # Increase difficulty by speeding up the aliens
        ...
        # Display the number of ship hits and misses
        ...
        screen.blit(misses_text, (screen.get_width() - misses_text.get_width() - 10, 50))
